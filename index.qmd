...
One thing we tested: simply increasing the top-k. The candidate set grows and the truth shows up more often — but the LLM then has more noise to filter through and its choice quality drops by roughly the same amount. The two effects cancel out: overall accuracy stays similar.

Take-away: RAG is genuinely viable when legacy labels don't exist, but to close the gap we need a better retriever — improved embeddings or a reranker — not just a wider net.
:::
