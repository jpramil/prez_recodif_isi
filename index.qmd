...
::: {.notes}
We tried no to rely on a single LLM. So, we launch the classification process with four open-source models in parallel — Qwen, Ministral, Mistral Small, and a quantised Llama seventy B. 
They differ in size, speed, performance, and how often they decline when uncertain.

Each one is used as an independent virtual annotator. We aggregate the votes — cascade, majority, weighted vote - to make the predictions more accurate. Same idea as any ensemble: complementary errors cancel out.
:::
...
::: {.notes}
Evaluation is hard. The reference is the thirty thousand annotated cases, which themselves have human disagreement. We use three metrics: overall accuracy, accuracy on codable cases, and LLM-only accuracy that factors out cases where the description was ambiguous or the legacy code itself was wrong. A naive evaluation underestimates the model substantially.
...
::: {.notes}
Preliminary numbers, to be consolidated. Pure RAG reaches accuracy close to RBAG — within a few percentage points, slightly lower. Small enough that RAG is a credible fallback when legacy labels aren't available. Full comparison in the journal version.
:::
